...
class DistilledVisionTransformer(VisionTransformer):
    ...
    def attention_rollout_save(self, x, indexes):
        B = x.shape[0]
        x_shape = x.shape  # Store the original shape for reshaping the final attention map

        # Initial embedding and token concatenation
        x = self.patch_embed(x)
        ...
        # Initialize the rollout attention matrix
        ...
        for blk in self.blocks:
            ...
            current_attention = att[:, :, :2, 2:].sum(dim=1).sum(dim=1)  # Sum without converting to numpy
            current_attention = current_attention.reshape([B, x_shape[2] // 16, x_shape[3] // 16])  # Keep as tensor

            # Rollout attention calculation by multiplying the current attention matrix with the accumulated one
            if rollout_attention is None:
                rollout_attention = current_attention
            else:
                rollout_attention = rollout_attention * current_attention

            x = blk(x)  # Continue processing within the block

        # Ensure directory exists and save the images
        if not os.path.exists(self.save):
            os.makedirs(self.save)

        last_rollout = rollout_attention
        for j, index in enumerate(indexes.cpu().numpy()):
            normalized_map = last_rollout[j] / last_rollout[j].max()
            normalized_map = normalized_map.cpu().numpy()  # Convert to numpy for saving
            plt.imsave(os.path.join(self.save, f'{index}.png'), np.tile(np.expand_dims(normalized_map, 2), (1, 1, 3)), cmap='viridis')
    ...
    def attention_flow_save(self, x, indexes, layer_start, layer_end):
        assert layer_start < layer_end < len(self.blocks), "Invalid layer indices"
        ...
        flow_attention = None
        for i, blk in enumerate(self.blocks):
            x, attn_weights = blk(x, return_attention=True)  # blk must return attention weights
            attn_weights = attn_weights.mean(dim=1)  # Average over heads

            if i == layer_start:
                flow_attention = attn_weights
            elif i > layer_start and i <= layer_end:
                flow_attention = torch.matmul(flow_attention, attn_weights)

            if i == layer_end:
                last_flow = flow_attention[:, 0, 1:].reshape(B, -1, self.patch_embed.grid_size[0], self.patch_embed.grid_size[1])
                last_flow = last_flow.sum(dim=1)  # Sum over heads
                break

        for j, index in enumerate(indexes.cpu().numpy()):
            plt.imsave(os.path.join(self.save, f'{index}.png'), last_flow[j].detach().cpu().numpy(), cmap='viridis')
    ...
    def forward(self, x, atten=None, indexes=None):
        if self.save is not None:
            # x, x_dist = self.forward_features_save(x, indexes) #For addition
            x, x_dist = self.attention_rollout_save(x, indexes) #For rollout
        elif self.crop:
            if atten is None:
