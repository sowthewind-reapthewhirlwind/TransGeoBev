...
parser.add_argument('--bev', action='store_true',
                    help='input is a BEV image')

parser.add_argument('--semantic', action='store_true',
                    help='input is a semantic image')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            ...
            if args.bev:
                ...
                # Assume new_pos_embed_shape is the shape your current model expects
                new_pos_embed_shape = [1, 258, 384]  # Example shape; adjust as needed

                # Resize pos_embed using interpolation or another appropriate method
                # This example uses simple reshaping for illustration; use interpolation as needed
                new_pos_embed = torch.nn.functional.interpolate(
                    checkpoint['state_dict']['module.reference_net.pos_embed'].unsqueeze(0),  # Add batch dim for interpolation
                    size=(new_pos_embed_shape[1], new_pos_embed_shape[2]),  # Target spatial size
                    mode='bilinear',  # Interpolation mode
                    align_corners=False
                ).squeeze(0)  # Remove batch dim after interpolation

                # Update the checkpoint
                checkpoint['state_dict']['module.reference_net.pos_embed'] = new_pos_embed


            model.load_state_dict(checkpoint['state_dict'])
            # if 'optimizer' in checkpoint and args.op == 'sam':
    ...
    if not args.multiprocessing_distributed or args.gpu == 0:
        ...
        if not os.path.exists(os.path.join(args.save_path, 'Ground Attention')):
            os.mkdir(os.path.join(args.save_path, 'Ground Attention'))
            os.mkdir(os.path.join(args.save_path, 'Ground Attention','train'))
            os.mkdir(os.path.join(args.save_path, 'Ground Attention','val'))
    ...
    if not args.crop:
        model.module.query_net.save = os.path.join(args.save_path, 'Ground Attention', 'train')
        scan(train_scan_loader, model, args)
        model.module.reference_net.save = os.path.join(args.save_path, 'Ground Attention', 'val')
        ...
        scan(val_scan_loader, model, args)
        with open(os.path.join(args.save_path, 'epoch_stats.json'), 'w') as f:
            json.dump(json_acc, f, indent=4)
...
def accuracy(query_features, reference_features, query_labels, topk=[1,5,10]):
    ...
    stats = {
    'Percentage-top1': results[0],
    'top5': results[1],
    'top10': results[2],
    'top1%': results[-1],  
    }
    json_acc.append(stats)
